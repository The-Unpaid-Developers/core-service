...
public class SystemComponentTest {
    ...
    class BuilderTests {
        ...
        @Nested
        class NewComponentBuilderTests {

            @Test
            void shouldApplyDefaults() {
                SystemComponent component = SystemComponent.newComponentBuilder("Inventory Service")
                        .role(ComponentRole.FRONT_END)
                        .hostedOn(Location.CLOUD)
                        .hostingRegion(HostingRegion.APAC)
                        .solutionType(SolutionType.COTS)
                        .languageFramework(dummyLanguageFramework())
                        .customizationLevel(CustomizationLevel.NONE)
                        .upgradeStrategy(UpgradeStrategy.INTERNAL_LED)
                        .upgradeFrequency(Frequency.MONTHLY)
                        .availabilityRequirement(AvailabilityRequirement.MEDIUM)
                        .scalabilityMethod(ScalabilityMethod.MANUAL)
                        .backupSite(BackupSite.NONE)
                        .securityDetails(dummySecurityDetails())
                        .build();

                assertEquals("Inventory Service", component.getName());
                assertEquals(ComponentStatus.NEW, component.getStatus());
                assertEquals(-1, component.getLatencyRequirement());
                assertEquals(-1, component.getThroughputRequirement());
                assertFalse(component.isInternetFacing());
                assertFalse(component.isSubscription());
                assertTrue(component.isOwnedByUs());
                assertTrue(component.isCICDUsed());
            }
        }
    }
